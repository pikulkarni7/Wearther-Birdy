...
def compute_suggestions(temperature: float, gender: str) -> dict:
    ...
    if temperature >= 24 and gender == "F":
        result["women"] = Product.objects.filter(
    ...
    if temperature < 24:
        __bottom__ = __bottom__.union(
            Product.objects.filter(category=bottom, weather_value__gt=2)  # pants
        )
    else:
        __bottom__ = __bottom__.union(
            Product.objects.filter(category=bottom, weather_value__lte=2)  # shorts
        )
    ...
    if __diff_temp__ <= 0:
        __top_must__ = __top_must__.union(
            Product.objects.filter(category=top_required, weather_value__lt=5)
        )
        ...
        if __diff_temp__ > 6:
            __top_optional_1__ = __top_optional_1__.union(
                Product.objects.filter(
                    category=top_optional_1, weather_value__lt=10, weather_value__gte=5
                )
            )

        if __diff_temp__ > 15:
            __top_optional_2__ = __top_optional_2__.union(
                Product.objects.filter(category=top_optional_2)
            )
